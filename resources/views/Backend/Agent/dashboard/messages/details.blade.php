      ...
      <div class="heading">
        <div class="container">
          <div class="row d-flex justify-content-center text-center">
            <div class="col-lg-8">
              <h1 class="heading-title">Agent Chat Box</h1>
              <p class="mb-0">
              </p>
            </div>
          </div>
      ...
      <nav class="breadcrumbs">
        <div class="container">
          <ol>
            ...
            <li class="current">Agent Chat Box</li>
          </ol>
        </div>
      </nav>
    </div><!-- End Page Title -->
...
<div class="chat-messages-wrapper" id="chat-scroll">
    @foreach ($mergedMessages as $chat)
        <div class="mb-3 {{ $chat->type }}">
            <div class="d-flex {{ $chat->type == 'sent' ? 'align-items-start gap-2' : 'justify-content-end' }}">

                {{-- User Avatar --}}
                @if ($chat->type == 'sent')
                    <img src="{{ asset('uploads/user/' . $user->photo) }}" class="chat-avatar" alt="User">
                @endif
    ...
    <div class="chat-input">
        <form class="d-flex gap-2" action="{{ route('agent_message_conversation',$message->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <input type="hidden" class="form-control" name="user_id" value="{{ $message->user_id }}">
            <textarea name="reply_conversation_body" id="message-input" rows="1" placeholder="Type a message..." class="form-control"></textarea>
